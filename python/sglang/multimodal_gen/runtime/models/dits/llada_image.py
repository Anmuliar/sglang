...
# Match the checkpoint's Diffusers SDPA numerics; FA remains an explicit override.
LLADA_IMAGE_DEFAULT_ATTENTION_BACKEND = AttentionBackendEnum.TORCH_CUDNN_SDPA


class LLaDAImageRMSNorm(SGLangRMSNorm):
    """Affine-free RMSNorm with the checkpoint's fp32-upcast reduction semantics."""
...
class LLaDAImageAttnProcessor:
    ...
    def __call__(
        ...
        if freqs_cis is not None:
            # Preserve Diffusers' complex64 rotary math before casting back to BF16.
...
class LLaDAImageAttention(nn.Module, AttentionModuleMixin):
    ...
    def __init__(
        ...
        self.sgl_attention = USPAttention(
            ...
            default_attention_backend=LLADA_IMAGE_DEFAULT_ATTENTION_BACKEND,
            prefix="llada_image",
        )
        self.norm_q = (
