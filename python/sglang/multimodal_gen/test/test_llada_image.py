...
from sglang.multimodal_gen.runtime.models.dits.llada_image import (
    LLADA_IMAGE_DEFAULT_ATTENTION_BACKEND,
    LLaDAImageFinalLayer,
    LLaDAImageRMSNorm,
...
class TestLLaDAImage(unittest.TestCase):
    ...
    def test_dit_rmsnorm_matches_fp32_upcast_reference(self):
        hidden_size = 128
        values = torch.linspace(
            -8.0,
            8.0,
            steps=3 * 5 * hidden_size,
            device="cuda",
            dtype=torch.float32,
        ).reshape(3, 5, hidden_size)
        hidden_states = values.to(torch.bfloat16)
    ...
    @unittest.skipUnless(torch.cuda.is_available(), "requires CUDA")
    def test_final_layer_norm_ignores_outer_autocast(self):
        layer = LLaDAImageFinalLayer(dim=16, out_channels=8).cuda().to(torch.bfloat16)
        hidden_states = torch.randn(2, 7, 16, device="cuda", dtype=torch.bfloat16)
        adaln_input = torch.randn(2, 16, device="cuda", dtype=torch.bfloat16)

        with torch.autocast("cuda", dtype=torch.bfloat16):
            scale = 1.0 + layer.adaLN_modulation(adaln_input).unsqueeze(1)
            with torch.autocast("cuda", enabled=False):
                normalized = layer.norm_final(hidden_states)
            expected = layer.linear(normalized * scale)
            actual = layer(hidden_states, adaln_input=adaln_input)
